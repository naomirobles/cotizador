...
// Tabla productos 
ipcMain.handle('agregar-producto', (event, id_cotizacion, precio_unitario, nombre, concepto, unidades, imagen = null) => {
  return new Promise((resolve, reject) => {
    db.run(`INSERT INTO Productos (id_cotizacion, precio_unitario, nombre, concepto, unidades, imagen) VALUES (?, ?, ?, ?, ?, ?)`, 
    [id_cotizacion, precio_unitario, nombre, concepto, unidades, imagen], function(err) {
      if (err) reject(err);
      else resolve(this.lastID);
    });
  });
